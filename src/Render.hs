...
module Render (createRendererInstance, renderGame) where
...
data RendererInstance = RendererInstance { glContext :: WebGL2RenderingContext
                                         , glProgram :: WebGLProgram
                                         , inputs :: Inputs
                                         , buffers :: Buffers
                                         }

createRendererInstance :: IO (Maybe RendererInstance)
createRendererInstance = do
  initHTML
  ...
  return (Just $ RendererInstance context shaderProg inputs buffers)

renderGame :: RendererInstance -> IO ()
renderGame (RendererInstance context prog inputs buffers) = do
  drawScene context prog inputs buffers
  return ()
...
initShaders context = do
  ...
  Just str2 <- getShaderInfoLog context fragmentShader 
  putStrLn str2
  ...
  Just str <- getShaderInfoLog context vertexShader 
  putStrLn str
  ...
  Just programLog <- getProgramInfoLog context shaderProgram
  putStrLn programLog
...
initShaderInputs cxt shaderProgram = do
  ...
  Just time <- getUniformLocation cxt shaderProgram' "time"
  enableVertexAttribArray cxt $ fromIntegral pos
  return $ Inputs (fromIntegral pos) (time)
